...
const WEAPONS = {
  AR: {
    ...
    fireDelay: 90, reload: 1200, spread: 0.022, range: 72, recoil: 0.006, pellets: 1, score: 100,
  },
  DMR: {
    label: 'PULSE DMR', type: 'DMR', color: 0xffd36c, damage: 54, headDamage: 110, mag: 12, reserve: 72,
    fireDelay: 310, reload: 1450, spread: 0.008, range: 100, recoil: 0.011, pellets: 1, score: 150,
  },
  SR: {
    label: 'ECLIPSE SR', type: 'SR', color: 0xff3159, damage: 125, headDamage: 260, mag: 5, reserve: 30,
    fireDelay: 850, reload: 1850, spread: 0.002, range: 150, recoil: 0.018, pellets: 1, score: 240,
  },
};
...
function shoot() {
  ...
  for (let i = 0; i < weapon.pellets; i += 1) {
    ...
    createTracer(raycaster.ray.origin, raycaster.ray.direction, bestHit?.distance || weapon.range, weapon.color);
  }
...
function createTracer(origin, direction, distance, color) {
  const geo = new THREE.BufferGeometry().setFromPoints([
    origin.clone().add(direction.clone().multiplyScalar(0.8)),
    origin.clone().add(direction.clone().multiplyScalar(Math.min(distance, 35))),
  ]);
  const mesh = new THREE.Line(geo, new THREE.LineBasicMaterial({ color, transparent: true, opacity: 0.9 }));
  scene.add(mesh);
  bullets.push({ mesh, life: 0.055 });
}
...
function updateProjectiles(dt) {
  bullets.forEach((bullet) => {
    bullet.life -= dt;
    bullet.mesh.material.opacity = Math.max(0, bullet.life / 0.055);
  });
  bullets.filter((bullet) => bullet.life <= 0).forEach((bullet) => scene.remove(bullet.mesh));
...
function updateHud() {
  ...
  fireMode.textContent = isReloading ? 'RELOADING...' : `${weapon.type} / ${Math.round(1000 / weapon.fireDelay * 60)} RPM`;
  drawRadar();
}
...
document.addEventListener('keydown', (event) => {
  ...
  if (event.code === 'Digit3') switchWeapon('SR');
...
setInterval(() => {
  if (mouse.down && weaponKey === 'AR') shoot();
}, 18);
